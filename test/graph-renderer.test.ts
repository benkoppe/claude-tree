...
describe("renderConversationGraph", () => {
  test("draws branches, endpoints, and selection status", () => {
    ...
    expect(rendered.text).toContain("U root question")
    expect(rendered.text).toContain("A main answer")
    expect(rendered.text).toContain("A fork answer")
    expect(rendered.text).toContain("> @ * live draft: fix the t...")
  ...
  test("centers a selected node inside a clipped viewport", () => {
    ...
    expect(rendered.text).toContain("> @ o saved [no live draft]")
  ...
  test("marks screen-derived drafts as approximate", () => {
    ...
    expect(rendered.text).toContain("> @ * live ~ draft: line on...")
  })
})
...
describe("renderRootPicker", () => {
  test("summarizes logical messages, leaves, and live families", () => {
    ...
    expect(rendered).toBe("> * Root  (3 messages, 2 leaves)")
    expect(renderRootPicker([], 0, 5, 80, new Set())).toContain("Press n")
  })
})
