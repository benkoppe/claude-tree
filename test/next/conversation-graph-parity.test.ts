...
import type { BranchRelation, ConversationRemoval } from "../../src/domain/persistence"
...
describe("buildConversationForest", () => {
  ...
  test("attaches a partially rewritten child at its last surviving shared message", () => {
    const parent = [
      message("q", "user", "question", 0),
      message("a", "agent", "answer", 1),
      message("follow-up", "user", "follow-up", 2),
      message("source", "agent", "source", 3),
    ]
    const copied = parent.map((entry) => ({ ...entry, id: `copy-${entry.id}` }))
    const graph = buildConversationForest(
      [session(ROOT, "Root", 20), session(CHILD, "Child", 10)],
      new Map([
        [ROOT, parent],
        [CHILD, [...copied.slice(0, 2), message("edited", "user", "edited", 2)]],
      ]),
      [relation(CHILD, ROOT, "source", shared(parent, copied, 4))],
    ).graphs[0]!
    const edited = messageNodes(graph).find((node) => node.preview === "edited")!
    const answer = messageNodes(graph).find((node) => node.preview === "answer")!
    expect(edited.parentId).toBe(answer.id)
