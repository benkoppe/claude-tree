...
Compaction is distinct from rewind. Claude transcript reads also import SDK session records within the same bounded read deadline to identify explicit compact summaries; summary wording is not evidence. Keep these records in the normalized transcript for identity and fork validation, but mark them as hidden compaction boundaries for graph projection. An unshared compact summary must not invalidate a preserved shared prefix, and a compacted fork's continuation stays at its recorded branch source even when compaction omits that prefix.
